...
ggplot(data=ibd,aes(x=Z0,y=Z1))+geom_point(alpha=1/4)+theme_bw() + xlim(0,1)
dev.off()
##pull duplicates
dups <- data.frame()
...
write.table(filter(others,PI_HAT>=args$pihat), args$outputdir %&% "/related.to.remove.txt", quote = FALSE, row.names = FALSE)
